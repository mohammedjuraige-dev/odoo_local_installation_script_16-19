...
echo "🚀 Starting Full Odoo 19 Installer on Ubuntu..."

# STEP 0: Set variables
ODOO_DIR=~/odoo19
ODOO_SOURCE_DIR=$ODOO_DIR/odoo
VENV_DIR=$ODOO_DIR/odoo19-venv
CONFIG_FILE=$ODOO_SOURCE_DIR/odoo19.conf
...
# STEP 3: Clone Odoo 19 source (remove old if exists)
if [ -d "$ODOO_SOURCE_DIR" ]; then
    echo "⚠️ Removing existing $ODOO_SOURCE_DIR folder for fresh clone..."
    rm -rf "$ODOO_SOURCE_DIR"
fi
echo "⬇️ Cloning Odoo 19 (branch 19.0) from GitHub..."
git clone --depth 1 --branch 19.0 https://github.com/odoo/odoo.git "$ODOO_SOURCE_DIR"
...
echo "👉 To run Odoo 19:"
echo "   cd $ODOO_DIR"
echo "   source odoo19-venv/bin/activate"
echo "   ./odoo/odoo-bin -c ./odoo/odoo19.conf"
