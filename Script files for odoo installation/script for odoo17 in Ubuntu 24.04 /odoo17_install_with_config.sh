...
echo "🚀 Starting Full Odoo 17 Installer on Ubuntu 24.04 or lower..."

# STEP 0: Set variables
ODOO_DIR=~/odoo17
ODOO_SOURCE_DIR=$ODOO_DIR/odoo
VENV_DIR=$ODOO_DIR/odoo17-venv
CONFIG_FILE=$ODOO_SOURCE_DIR/odoo17.conf
...
sudo apt update && sudo apt install -y \
  ...
  libopenjp2-7-dev libssl-dev zlib1g-dev liblcms2-dev libfreetype-dev \
  libwebp-dev libharfbuzz-dev libfribidi-dev libxcb1-dev libpng-dev \
  libblas-dev libatlas-base-dev python3-pip postgresql

# STEP 3: Clone Odoo 17 source
if [ -d "$ODOO_SOURCE_DIR" ]; then
  echo "⚠️  $ODOO_SOURCE_DIR already exists."
...
echo "⬇️ Cloning Odoo 17 from GitHub into $ODOO_SOURCE_DIR..."
git clone --depth 1 --branch 17.0 https://github.com/odoo/odoo.git "$ODOO_SOURCE_DIR"
...
if [ -f "$ODOO_SOURCE_DIR/requirements.txt" ]; then
  ...
  echo "⚠️ requirements.txt not found at $ODOO_SOURCE_DIR. Skipping Python dependency install."
fi
...
echo "👉 To run Odoo:"
echo "   cd $ODOO_DIR"
echo "   source odoo17-venv/bin/activate"
echo "   ./odoo/odoo-bin -c ./odoo/odoo17.conf"
mkdir -p "$ODOO_SOURCE_DIR/custom_addons"
